...
#[cfg(test)]
mod tests {
    use super::{matrixTrait, vecTrait, mapper, reducer, PrintTrait, final_output};
    #[test]
    ...
    fn testing_init_arr() {
        ...
        let (key, first_row_sum) = reducer(0, @map_res);
        assert(first_row_sum == 6, 'result should be 6');
        let (key, second_row_sum) = reducer(1, @map_res);
        assert(second_row_sum == 15, 'result should be 15');
        let (key, third_row_sum) = reducer(2, @map_res);
